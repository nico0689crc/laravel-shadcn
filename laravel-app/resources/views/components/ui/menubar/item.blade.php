...
$variantClass = $variant === 'destructive'
    ? 'text-destructive hover:bg-destructive/10 hover:text-destructive focus:bg-destructive/10 focus:text-destructive data-[highlighted]:bg-destructive/10 data-[highlighted]:text-destructive [&_svg]:text-destructive'
...
@if($href)
    <a
        ...
        @if($closeOnClick) @click="_close()" @endif
...
@else
    <div
        ...
        tabindex="-1"
        @if($disabled) aria-disabled="true" data-disabled @endif
        @if(!$disabled && $closeOnClick) @click="_close()" @endif
